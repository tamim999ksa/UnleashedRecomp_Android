...
# Completely rewrite find functions
new_symbol_table = """#pragma once
#include "symbol.h"
#include <set>
#include <iterator>
...
with open('tools/XenonRecomp/XenonUtils/symbol_table.h', 'w') as f:
    f.write(new_symbol_table)

# 3. xex.h - update getOptHeaderPtr signature
...
# 4. Update calls to getOptHeaderPtr in xex.cpp and xex_patcher.cpp
patch_file('tools/XenonRecomp/XenonUtils/xex.cpp', r'getOptHeaderPtr\(data,', 'getOptHeaderPtr(data, dataSize,')
patch_file('tools/XenonRecomp/XenonUtils/xex_patcher.cpp', r'getOptHeaderPtr\(patchBytes,', 'getOptHeaderPtr(patchBytes, patchSize,')
patch_file('tools/XenonRecomp/XenonUtils/xex_patcher.cpp', r'getOptHeaderPtr\(xexBytes,', 'getOptHeaderPtr(xexBytes, xexSize,')
...
# 5. xex.cpp - use emplace(...) instead of insert({...})
# Specifically for image.symbols.insert({ ... })
# Pattern: image.symbols.insert({ name->second, descriptors[im].firstThunk, sizeof(thunk), Symbol_Function });
patch_file('tools/XenonRecomp/XenonUtils/xex.cpp',
           r'image\.symbols\.insert\(\{(.*?)\}\);',
           r'image.symbols.emplace(\1);')

# 6. aggregate return fixes
...
# Scoping fix: declare dataStart/dataEnd first
