...
# 4. xex.cpp & xex_patcher.cpp - getOptHeaderPtr calls & emplace
patch_file('tools/XenonRecomp/XenonUtils/xex.cpp', r'getOptHeaderPtr\(data,', 'getOptHeaderPtr(data, dataSize,')
# Use simpler emplace replacement
with open('tools/XenonRecomp/XenonUtils/xex.cpp', 'r') as f: content = f.read()
...
with open('tools/XenonRecomp/XenonUtils/xex.cpp', 'w') as f: f.write(content)
...
# We'll just replace the whole Function::Analyze function start to avoid scope errors
with open('tools/XenonRecomp/XenonAnalyse/function.cpp', 'r') as f: content = f.read()
old_start = r'Function Function::Analyze\(const void\* code, size_t size, size_t base\)\s+\{.*?auto& blocks = fn\.blocks;'
...
content = re.sub(r'return fn;\s+\}', '    if (fn.size == 0 && data > dataStart) fn.size = 4;\n    return fn;\n}', content)
